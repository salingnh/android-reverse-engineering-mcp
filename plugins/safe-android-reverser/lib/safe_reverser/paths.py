...
def atomic_write_json(directory: Path, filename: str, payload: dict[str, Any], *, max_bytes: int) -> None:
    ...
    temp: Path | None = None
    try:
        with tempfile.NamedTemporaryFile(dir=directory, prefix=f".{filename}.", suffix=".tmp", delete=False) as handle:
...
def read_json_file(directory: Path, filename: str, *, max_bytes: int) -> dict[str, Any]:
    directory = directory.resolve()
